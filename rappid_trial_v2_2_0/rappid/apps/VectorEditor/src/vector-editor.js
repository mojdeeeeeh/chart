...
app.Path = joint.shapes.basic.Path.define('Path', {
    attrs: {
        path: {
            strokeMiterlimit: 4,
    ...
    updateBBox: function(paper, bbox, opt) {
    ...
    updatePathData: function(paper) {
        ...
        this.attr('path/d', path.attr('d'), {
            nextBBox: path.getBBox({ target: view.rotatableNode }).offset(this.position()),
    ...
    createFromNode: function(pathNode, paper) {
        ...
        var p = new this({
            ...
            attrs: { path: { d: pathNode.getAttribute('d') }}
        });
    ...
    attributes: {
        d: {
            set: function(d, bbox, el) {
                // automatically normalize path upon setting d
                // take a d string and set it to the el element
...
app.AppView = joint.mvc.View.extend({
    ...
    initControllers: function() {
        ...
        this.graph.on({
            ...
            'change:attrs': function(cell, attrs, opt) {
                if (opt.propertyPath === 'attrs/path/d') {
                    var bbox = opt[(opt.revert) ? 'prevBBox' : 'nextBBox'];
                    if (bbox) {
                        cell.updateBBox(this.paper, bbox, { dry: true });
                    }
                }
            }
        }, this);
    ...
    downloadFile: function(data, fileName, mimeType) {
        ...
        var href;
        if (_.startsWith(data, 'data:')) {
            href = data;
        } else {
            href = 'data:' + (mimeType || 'text/xml') + ',' + encodeURIComponent(data);
        }

        if (window.navigator.msSaveBlob) {
            window.navigator.msSaveBlob(this.dataUriToBlob(href), fileName);
        } else {
            var $anchor = $('<a>', { download: fileName, href: href });
    ...
    dataUriToBlob: function(dataURI) {
        ...
        var encodedString = dataURI.slice(firstCommaIndex + 1);

        var byteString;
        if (dataTypeString.indexOf('base64') >= 0) {
            byteString = atob(encodedString);
        } else {
